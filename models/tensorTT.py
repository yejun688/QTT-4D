...
class TensorTT(TensorBase):
    ...
    def init_svd_volume(self, res, device):
        ...
        if self.fused:
            ...
            self.vox_sigma = QTT3dQuimb(res, max_rank_tt = max_rank_density, use_TTNF_sampling=self.use_TTNF_sampling,
                                    payload_dim = 1,
                                    compression_alg = self.compression_alg, canonization = self.canonization) # 密度，创建了一个QTT3dQuimb类的对象
    ...
    def upsample_volume_grid(self, reso_target):
        ...
        self.num_upsamples_perfomed += 1 # 执行上采样的次数做更新
    ...
    def get_optparam_groups(self, lr_init_spatial = 0.003, lr_init_network = 0.001, lr_init_featurenet = 0.0008, lr_init_deformation_net = 0.0006, lr_init_density_net = 0.0008, lr_init_time_net = 0.0008):
        """
        ...
        out = []
        out += [
            {'params': self.timenet.parameters(), 'lr':  lr_init_time_net}
        ]
        out += [
            {'params': self.time_space_attention.parameters(), 'lr':  lr_init_deformation_net}
        ]
        out += [
            {'params': self.deformation_net.parameters(), 'lr':  lr_init_deformation_net}
        ]
        if self.fused:
            out += [
        ...
        out += [
            {'params': self.feature_net.parameters(), 'lr':  lr_init_featurenet}
        ]
        out += [
            {'params': self.density_net.parameters(), 'lr':  lr_init_density_net}
        ]
    ...
    def get_compression_values(self):
        ...
        """
        # 从 self.vox_fused 中获取未压缩和压缩的参数、大小
        if self.fused:
        ...
        # 从 self.vox_rgb 和 self.vox_sigma 中分别获取并相加未压缩和压缩的参数、大小
        else:
