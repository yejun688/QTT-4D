...
class QTT3dQuimb(torch.nn.Module):
    ...
    def get_contraction_expression(self, only_expression = False):
        ...
        """
        output_inds = self.tn.outer_inds() # ('payload', 'k0', 'k1', 'k2', 'k3')
        output_inds = [ind for ind in output_inds if ind != 'payload'] # remove string "payload" from output inds ['k0', 'k1', 'k2', 'k3']S
        output_inds = ['payload'] + output_inds # add to front of output inds ['payload', 'k0', 'k1', 'k2', 'k3']这几行将数据格式转成列表
        backend = 'torch'
        optimze = 'auto-hq'
        if only_expression:
        ...
        symbol_info =self.tn.contract( output_inds=output_inds, get='symbol-map', backend = backend, optimize=optimze) # {'payload': 'a', 'k0': 'b', 'virt0': 'c', 'k1': 'd', 'virt1': 'e', 'k2': 'f', 'virt2': 'g', 'k3': 'h'}
        contraction_expression = self.tn.contract(output_inds=output_inds, get='expression', backend=backend, optimize=optimze)
    ...
    def init_tn(self):
        """
        在tensorTT中创建QTT3dQuimb的一个对象self.vox_sigma时自动执行，在init里面
    ...
    def get_torch_params(self, tn):
        ...
        """
        params, self.skeleton = qtn.pack(tn) # self.skeleton：TensorNetwork(tensors=4, indices=8)
    ...
    def set_compression_variables(self):
        ...
        self.num_uncompressed_params =  np.prod(self.shape_source) * self.payload_dim # 160680
        self.num_compressed_params = sum(p.numel() for p in self.torch_params.values()) # 110592
    ...
    def update_contraction_costs(self, contraction_info = None):
        ...
        self.flops = contraction_info.opt_cost # Decimal('61710336')
        self.largest_intermediate = contraction_info.largest_intermediate # Decimal('110592')
        self.flops_per_iter = self.flops_per_iter + [ self.flops] # [Decimal('61710336')]
        self.largest_intermediate_per_iter = self.largest_intermediate_per_iter + [ self.largest_intermediate] # [Decimal('110592')]
