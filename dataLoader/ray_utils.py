...
def get_ray_directions(H, W, focal, center=None):
    ...
    """
    grid = create_meshgrid(H, W, normalized_coordinates=False)[0] + 0.5 # 此时为像素坐标系
...
def get_ray_directions_blender(H, W, focal, center=None):
    ...
    # see https://github.com/bmild/nerf/issues/24
    cent = center if center is not None else [W / 2, H / 2]
    # xy坐标按照已经投影到平面的相机坐标计算，torch.ones_like(i) 表示 z 方向的单位向量（沿着视线方向为额外加入的维度
...
def get_rays(directions, c2w):
    ...
    # # c2w[:3, 3]第三列，前三行为相机坐标系原点在世界坐标系中的位置
    rays_o = c2w[:3, 3].expand(rays_d.shape)  # (H, W, 3)

    rays_d = rays_d.view(-1, 3) # 将射线方向的张量从 (H, W, 3) 重塑为 (H*W, 3)
    rays_o = rays_o.view(-1, 3) # 将射线原点的张量从 (H, W, 3) 重塑为 (H*W, 3)
