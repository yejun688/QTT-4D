...
class ImmersiveDataset(Base6DDataset):
    def __init__(self, datadir, split="train", downsample=1.0, is_stack=False):
        ...
        self.num_frames = 50
        self.start_frame = 50
        self.keyframe_step = 4
        self.num_keyframes = (
            self.num_frames // self.keyframe_step
        )
        ...
        self.subsample_frac = 0.125

        self.keyframe_offset = 0
        self.frame_offset = 0

        super().__init__(datadir, split=self.split, downsample=1.0, is_stack=False)
    ...
    def read_meta(self):
        ...
        for idx, camera in enumerate(self.meta):
            ...
            self.focals.append(camera["focal_length"] * width_factor)
        ...
        # self.intrinsics = np.stack([self.intrinsics for i in range(self.num_frames)])
        # self.distortions = np.stack([self.distortions for i in range(self.num_frames)])
    ...
    def prepare_train_data(self):
        ...
        # sam_checkpoint = '/data/PuTT/PuTT-Nerf/pre_trained/sam_hq_vit_h.pth' # Update this path to your checkpoint
        # model_type = "vit_h"
        ...
        for video_idx in range(len(self.video_paths)):
            ...
            print(f"Video {video_idx} has {total_frames} frames.")
            # # Get coords
            # video_coords, rays_temp = self.get_coords(video_idx)  # torch.Size([1228800, 8]) torch.Size([1228800, 6])

            ctr = 0
            frame_idx = 0
            
            # Get coords
            video_coords, rays_o, rays_d = self.get_coords(video_idx * self.num_frames)  # torch.Size([1228800, 8]) torch.Size([1228800, 6])
            video_times = torch.tensor([i / (self.num_frames - 1) for i in range(self.num_frames)])
            all_times += [video_times]
            W, H = self.img_wh
            
            if self.split == 'train':                   
                rays_o, rays_d = ndc_rays_blender(H, W, self.focals[video_idx+1], 1.0, rays_o, rays_d)
            else:
                rays_o, rays_d = ndc_rays_blender(H, W, self.focals[video_idx], 1.0, rays_o, rays_d)
            # 记录cur_coords的维度1-3，5-7 到all_rays
            all_rays += [torch.cat([rays_o, rays_d], 1)]
            video_rgb = []
            while ctr < self.start_frame + self.num_frames:   
                _, frame = cam.read()  # 自动读取下一帧
                if ctr < self.start_frame:
                    ctr += 1
                    continue
                else:
                ...
                # if frame_idx == 5:
                #     cur_rgb_full = cur_rgb_full.reshape(H, W, 3)  # 将其重塑为 (H, W, 3)

                #     cur_rgb_full = cur_rgb_full * 255.0


                #     cur_rgb_full = cur_rgb_full.byte().cpu().numpy()

                # # 保存图像
                #     cv2.imwrite("output_image.png", cur_rgb_full)
                
                # # 恢复到原始范围 (假设归一化是到 [0, 1] 的范围)
                #     img_cropped = img_cropped * 255.0  # 归一化恢复

                #     img_cropped = img_cropped.permute(1, 2, 0).byte().cpu().numpy()

                #     cv2.imwrite("cropped_image.png", img_cropped)
                #     exit()
                video_rgb.append(cur_rgb_full.unsqueeze(0))
            ...
            cam.release()
            all_rgbs.append(torch.cat(video_rgb, dim=0))
        exit()
        # del sam
        # del predictor

        # # Format / save loaded data
        # self.all_coords = torch.cat(self.all_coords, 0) # torch.Size([912184655, 8])
        # self.all_rgb = torch.cat(self.all_rgb, 0) # torch.Size([912184655, 3])
        # self.all_sam_coords = torch.cat(self.all_sam_coords, 0) # torch.Size([9011200, 8])
        # self.all_sam = torch.cat(self.all_sam, 0) # torch.Size([9011200, 256])
        
        # self.time_temps = np.array(self.time_temps).astype(np.float32)
        # self.all_times.append(self.time_temps)
        # self.all_times = np.concatenate(self.all_times, 0)
        all_times = torch.stack(all_times, 0)
        self.all_times = all_times.view(self.images_per_frame, self.num_frames, 1)
        
        if not self.is_stack:
            all_rays = torch.stack(all_rays, 0)
            self.all_rays = all_rays.reshape(self.images_per_frame, W*H, 6)
            # self.all_rays = torch.cat(self.all_rays, 0)  # (len(self.meta['frames])*h*w, 6)
            # self.all_rgbs = torch.cat(self.all_rgbs, 0)  # (len(self.meta['frames])*h*w, 3)
            self.all_rgbs = torch.stack(all_rgbs, dim=0) 
            print(self.all_rays.shape) # torch.Size([4, 1228800, 6])
            print(self.all_rgbs.shape) # torch.Size([4, 50, 1228800, 3])
            self.global_mean_rgb = torch.mean(self.all_rgbs, dim=1)
        else:
