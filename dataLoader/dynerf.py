...
class Neural3D_NDC_Dataset(Dataset):
    ...
    def load_meta(self):
        ...
        if self.split == "train":
            ...
            all_times = torch.stack(all_times, 0) # torch.Size([17, 300])
            all_rays = torch.stack(all_rays, 0) # torch.Size([17, 786432, 6])
            # breakpoint()
            print("stack performed")
            N_cam, N_time, N_rays, C = all_imgs.shape # 17, 300, 786432, 3
            self.image_stride = N_rays # 786432
            self.cam_number = N_cam # 17
            self.time_number = N_time # 300
            self.all_rgbs = all_imgs # torch.Size([17, 300, 786432, 3])
            self.all_times = all_times.view(N_cam, N_time, 1) # torch.Size([17, 300, 1])
            self.all_rays = all_rays.reshape(N_cam, N_rays, 6) # torch.Size([17, 786432, 6])
            self.all_times = self.time_scale * (self.all_times * 2.0 - 1.0) # torch.Size([17, 300, 1])
