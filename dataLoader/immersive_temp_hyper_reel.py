...
class ImmersiveDataset(Base6DDataset):
    ...
    def prepare_train_data(self):
        ...
        for video_idx in range(len(self.video_paths)):
            ...
            while ctr < self.start_frame + self.num_frames:
                ...
                cur_coords = torch.cat(
                    ...
                    -1
                ) # ideo_coords[..., :-1]的维度为torch.Size([1228800, 7]),每一帧的时间需要重新确定，之后cur_coords的维度为torch.Size([1228800, 8])
                ...
                # Rays
                self.all_rays += [cur_coords[..., :6]]
                
                # Camera_ids
                self.all_camera_ids += [cur_coords[..., 6:7]]  
                
                # Times
                self.all_times += [cur_coords[..., 7:8]]  

                # Coords
                self.all_coords += [cur_coords]

                # Color
                self.all_rgbs += [cur_rgb]
        ...
        self.all_coords = torch.cat(self.all_coords, 0) # torch.Size([883355514, 8])
        self.all_rays = torch.cat(self.all_rays, 0) # torch.Size([883355514, 6])
        self.all_times = torch.cat(self.all_times, dim=0) # torch.Size([883355514, 6])
        ...
        self.all_rgbs = torch.cat(self.all_rgbs, 0) # torch.Size([883355514, 3])
    ...
    def prepare_test_data(self):
        ...
        for video_idx in range(len(self.video_paths)):
            ...
            while ctr < self.start_frame + self.num_frames:
                ...
                # Color
                self.all_rgbs += [cur_rgb_full]

                # Number of pixels
                num_pixels += cur_rgb_full.shape[0]
        ...
        # Format / save loaded data
        self.all_coords = torch.cat(self.all_coords, 0) # torch.Size([61440000, 8])
        # self.all_times = torch.cat(self.all_times, dim=0) # torch.Size([61440000, 1])
        
        # self.update_all_data()
        self.all_rays = torch.stack(self.all_rays, 0)  # torch.Size([50, 1228800, 6])
        self.all_rgbs = torch.stack(self.all_rgbs, 0).reshape(-1,*self.img_wh[::-1], 3)  # torch.Size([50, 960, 1280, 3])
