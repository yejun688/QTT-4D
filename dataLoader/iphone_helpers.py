...
def load_iphone_cameras(dir_path):
    ...
    all_cam_ids = np.unique(all_cam_ids).tolist() # 加载筛选所有的cam_id
    ...
    for cam_id in all_cam_ids:
        ...
        for fn in tqdm(cam_fns):
            ...
            cy_ratio = principal_point[1] / image_size[1] # 计算一个比率，相机主点和实际尺寸
...
def load_iphone_gt_poses(src, t_subsample):
    gt_training_cams = load_iphone_cameras(osp.join(src, "cameras")) # 获得一个字典，包含变换矩阵，视野角的度数，x，y方向中心相比这两个方向长度的比例，时间，以及文件名

    assert (
        len(gt_training_cams) == 1
    ), "Only support Mono camera for now in Iphone dataset"
    # subsample the gt camera
    gt_training_cams = {k: v[::t_subsample] for k, v in gt_training_cams[0].items()} # 选取第一个相机字典的key和value进行步长为subsample的下采样,这里的训练cam只有一个
    gt_testing_cams = load_iphone_cameras(osp.join(src, "test_cameras")) # 测试的cams有两个，索引为1，2
    gt_training_fov = gt_training_cams["fovdeg"][0] # 52.071839030522504
    logging.info(
        f"Iphone gt training camera fov is {gt_training_cams['fovdeg'][0]:.3f} deg."
    )
    gt_training_cam_T_wi = gt_training_cams["T_wi"] # torch.Size([475, 4, 4])
    # ! this bound which test frames can be retrieved
    gt_training_tids = gt_training_cams["t"] # 长度475
    gt_training_cxcy_ratio = gt_training_cams["cxcy_ratio"] # 475个2维的列表
